...
namespace sclade
{
    public partial class newcountry_of_origin : Form
    {
        public int id;
        public string code;
        public string name;
        ...
        public newcountry_of_origin(NpgsqlConnection con, int id, string code, string name, string litter)
        {
            this.con = con;
            this.id = id;
            this.name = name;
        ...
        private void newcountry_of_origin_Load(object sender, EventArgs e)
        {
            try
            {
    ...
    public void Update()
    {
            ...
            {
                String sql = "Select *  from country_of_origin where id=:id ORDER BY code ASC;";
                NpgsqlDataAdapter da = new NpgsqlDataAdapter(sql, con);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (this.id == -1)
            {
                try
                {
                    ...
                    string sql = "Insert into country_of_origin (code,name,litter) values(:code,:name,:litter);";
            ...
            else
            {
                try
                {
                    string sql = "update country_of_origin  set code=:code,name=:name, litter=:litter  where id=:id;";
                    NpgsqlCommand command = new NpgsqlCommand(sql, con);
                    command.Parameters.AddWithValue("code", textBox1.Text);
                    command.Parameters.AddWithValue("litter", textBox5.Text);
