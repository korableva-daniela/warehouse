...
namespace sclade
{
    public partial class new_product_place : Form
    {
        ...
        public int id_em;
        public int div;
        public new_product_place(NpgsqlConnection con, int id_invoices, string stor, int ind, int id_em, int quantity, int all_quantity, int div)
        {
            this.div = div;
        ...
        public void Update()
        {
            try
            {
                if (this.ind == 0)
                {
                    ...
                    dataGridView3.Columns[11].HeaderText = "Количество размещенного товара";
                }
                else
                {
                    if (this.ind == 1)
                    {
                        String sql1 = "Select storehouse.name,moving.num_invoices,storehouse.id from moving,storehouse where moving.id_storehouse_1=storehouse.id and moving.id = " + this.id_invoices;
                        NpgsqlDataAdapter da1 = new NpgsqlDataAdapter(sql1, con);
                        ds1.Reset();
                        da1.Fill(ds1);
                        dt1 = ds1.Tables[0];
        ...
        private void new_product_place_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView3.CurrentRow.Cells[0].Value != null && dataGridView3.CurrentRow != null)
                {
                    int id_c = 0;
                    string number = "";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    sql9 += st.ToString();
                    sql9 += " and num_place = '" + textBox2.Text + "/" + textBox3.Text + "/" + textBox4.Text + "/" + textBox5.Text + "'";
                    ...
                    if (dt9.Rows.Count > 0)
                    {
                        id_prod_storehouse = Convert.ToInt32(dt9.Rows[0]["id"]);
                        ...
                        if (result2 == DialogResult.Yes)
                        {
                            ...
                            command3.Parameters.AddWithValue("id_prod_storehouse", id_prod_storehouse);
                            command3.Parameters.AddWithValue("id_Product_card", comboBox1.SelectedValue);
                            command3.Parameters.AddWithValue("id_batch_number", comboBox2.SelectedValue);
                    ...
                    else
                    {
                        ...
                        string str_place = textBox2.Text + "/" + textBox3.Text + "/" + textBox4.Text + "/" + textBox5.Text;
                        command2.Parameters.AddWithValue("num_place", str_place);
                        command2.Parameters.AddWithValue("id_store", st);
                        ...
                        DialogResult result2 = MessageBox.Show("Вы уверены, что хотите добавить запись?", "Выполнение операции", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                            if (result2 == DialogResult.Yes)
                            {
                                ...
                                sql11 += st.ToString();
                                sql11 += " and num_place = '" + textBox2.Text + "/" + textBox3.Text + "/" + textBox4.Text + "/" + textBox5.Text + "'";
                                ...
                                {
                                    id_prod_storehouse = Convert.ToInt32(dt10.Rows[0]["id"]);
                                    ...
                                    if (this.ind == 0)
                                    {
                                        ...
                                        command4.Parameters.AddWithValue("id", id_invoices_info);
                                        command4.ExecuteNonQuery();
                                        //if (quantity == all_quantity)
                                        //{
                                        //    string sql5 = "update invoices_in set status=:status where id=:id";
                                        //    NpgsqlCommand command5 = new NpgsqlCommand(sql5, con);
                                        //    command5.Parameters.AddWithValue("status", "Обработан");
                                        //    command5.Parameters.AddWithValue("id", id_invoices);
                                        ...
                                        //    string sql5 = "update invoices_in set status=:status where id=:id";
                                        //    NpgsqlCommand command5 = new NpgsqlCommand(sql5, con);
                                        //    command5.Parameters.AddWithValue("status", "В обработке");
                                        //    command5.Parameters.AddWithValue("id", id_invoices);
                                        //    command5.ExecuteNonQuery();
                                        //}
                                        Update();
                                    ...
                                    if (this.ind == 1)
                                    {
                                        ...
                                        //if (quantity == all_quantity)
                                        //{
                                        //    string sql5 = "update moving set status=:status where id=:id";
                                        //    NpgsqlCommand command5 = new NpgsqlCommand(sql5, con);
                                        //    command5.Parameters.AddWithValue("status", "Обработан");
                                        //    command5.Parameters.AddWithValue("id", id_invoices);
                                        //    command5.ExecuteNonQuery();
        ...
        //        {
        //            num = dt12.Rows[0]["num_invoices"].ToString();
        ...
        private void информацияОНакладнойToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.ind == 0)
                {
                    ...
                    invoices_in fp = new invoices_in(con, st, this.id_em, num, 0, this.div);
                    fp.ShowDialog();
                }
                if (this.ind == 1)
                {
