...
namespace sclade
{
    public partial class newmoving_info : Form
    {
        ...
        int st;
        public int div;
        public newmoving_info(NpgsqlConnection con, int id, int id_invoices, string id_Product_card, string id_batch_number, int quantity, string stor,int div)
        {
            this.div = div;
            this.id = id;
        ...
        public void Update()
        {
            try
            {
                ...
                sql6 += this.id_invoices.ToString();

                NpgsqlDataAdapter da10 = new NpgsqlDataAdapter(sql6, con);
                ds10.Reset();
                da10.Fill(ds10);
                dt10 = ds10.Tables[0];
        ...
        private void newmoving_info_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                sql19 += st.ToString();
                sql19 += " and prod_store.count>0 ORDER BY prod_store.count ASC;";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (this.id == -1)
            {
                try
                {
                    ...
                    {
                        string sql = "Insert into moving_info ( invoices_in,id_Product_card,id_batch_number,quantity,count) values ( :invoices_in,:id_Product_card,:id_batch_number,:quantity,0)";
            ...
            else
            {
                try
                {
                    ...
                    NpgsqlDataAdapter da21 = new NpgsqlDataAdapter(sql18, con);
                    ds21.Reset();
                    da21.Fill(ds21);
                    dt21 = ds21.Tables[0];
                    if (dt21.Rows.Count > 0)
                    {
                        count = Convert.ToInt32(dt21.Rows[0]["count"]);
                    }
                    ...
                    else
                    {
                        ...
                        string sql = "update moving_info  set invoices_in=:invoices_in, id_Product_card=:id_Product_card, id_batch_number=:id_batch_number," +
                       "quantity=:quantity " +
