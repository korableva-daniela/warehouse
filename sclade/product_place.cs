...
namespace sclade
{
    public partial class product_place : Form
    {
        ...
        public int div;
        public product_place(NpgsqlConnection con, int stor, string code, int id_em, int pro,int div)
        {
            this.div = div;
            this.code = code;
            this.id_em = id_em;
        ...
        private void product_place_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            if (messages.Count == 0)
            {
                if ((comboBox1.Text != "Склад не выбран"))
                {
                    ...
                    else
                    {
                        ...
                        dataGridView1.Columns[4].HeaderText = "Количество размещенного товара";
            ...
            try
            {
                if (dataGridView1.CurrentRow != null)
                {
                    ...
                    if (type == "Перемещение на склад")
                    {
                        ...
                        dataGridView2.Columns[9].HeaderText = "Количество размещенного товара";
                    }
                    else
                    {
                        if (type == "Приходная")
...
"JOIN " +
"    storehouse s ON i.id_storehouse = s.id  where (i.status='Не указано' or i.status='В обработке') and i.flag = 0  and s.id_div = " + this.div.ToString() + "  and i.shipment <='" + DateTime.Today.Date + "'  GROUP BY i.id, i.num_invoices, i.shipment, s.name, i.flag " +
...
"JOIN " +
"   storehouse s2 ON m.id_storehouse_2 = s2.id   where (m.status='Не указано' or m.status='В обработке') and s2.id_div = " + this.div.ToString() + "   and m.shipment_to <= '" + DateTime.Today.Date + "'  GROUP BY m.id, m.num_invoices, m.shipment, s2.name  ORDER BY shipment_date DESC";
                    ...
                    else
                    {
                        if (dataGridView1.CurrentRow.Cells[6].Value != null&& dataGridView1.Rows[0].Cells[0].Value!=null)
                        {
                            ...
                            else
                            {
                                ...
                                if ((string)dataGridView1.CurrentRow.Cells[6].Value == "Перемещение на склад")
                                {
        ...
        private void добавитьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int all_quantity = Convert.ToInt32(dataGridView1.CurrentRow.Cells[3].Value);
                    ...
                    if ((string)dataGridView1.CurrentRow.Cells[6].Value == "Приходная")
                    {
                        new_product_place f = new new_product_place(con, num_invoices, stor, 0, this.id_em, Convert.ToInt32(quantity), Convert.ToInt32(all_quantity), this.div);
                        f.ShowDialog();
                    }
                    if ((string)dataGridView1.CurrentRow.Cells[6].Value == "Перемещение на склад")
                    {
                        new_product_place f = new new_product_place(con, num_invoices, stor, 1, this.id_em, Convert.ToInt32(quantity), Convert.ToInt32(all_quantity), this.div);
                        f.ShowDialog();
                    }
                    Update();
