...
namespace sclade
{
    public partial class newinvoices_out : Form
    {
        ...
        int id_i;
        public int div;
        public newinvoices_out(NpgsqlConnection con, int id, string num_invoices, string id_Firm, string id_storehouse, string num_Contract, double total_sum, double total_sum_nds, string status, int id_Employee, DateTime data, DateTime shipment, int address,int div)
        {
        ...
        private void newinvoices_out_Load(object sender, EventArgs e)
        {
            try
            {
                Update();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                firm_client fp = new firm_client(con, id_f, name, (int)comboBox2.SelectedValue, this.div);
                fp.ShowDialog();
                if (fp.name != "")
                {
                ...
                {
                    comboBox1.Text = "Клиент не выбран";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (this.id == -1)
            {
                ...
                try
                {
                    ...
                    string sql = "Insert into invoices_in (num_invoices,id_Firm, id_storehouse,data,num_Contract,total_sum,total_sum_nds,shipment,status,id_Employee,flag,Address_f) values (:num_invoices, :id_Firm, :id_storehouse, :data, :num_Contract, :total_sum,:total_sum_nds, :shipment, :status, :id_Employee, 1,:Address_f)";
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (result1 == DialogResult.Yes)
                        {
                            ...
                            else
                            {
                                ...
                                MessageBox.Show("Расходная накладная не найдена.");
                            }
                            this.StartPosition = FormStartPosition.CenterScreen;
                            newinvoices_out_info f = new newinvoices_out_info(con, -1, id_i, "", "", 0, 0, comboBox2.Text, this.div);
                            f.ShowDialog();
                            Update();
                            Close();
                ...
                try
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (result1 == DialogResult.Yes)
                        {
                            ...
                            newinvoices_out_info f = new newinvoices_out_info(con, -1, this.id, "", "", 0, 0, comboBox2.Text, this.div);
                            f.ShowDialog();
                            Update();
                            Close();
                        }
